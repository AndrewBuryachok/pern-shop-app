...
export class WaresService {
  ...
  async selectWareStates(wareId: number): Promise<WareState[]> {
  ...
  private getWaresQueryBuilder(req: Request): SelectQueryBuilder<Ware> {
    return this.waresRepository
      ...
      .loadRelationCountAndMap('ware.states', 'ware.states')
      .where(
        new Brackets((qb) =>
          qb.where(`${!req.id}`).orWhere('ware.id = :id', { id: req.id }),
        ),
      )
      .andWhere(
        new Brackets((qb) =>
      ...
      .andWhere(
        new Brackets((qb) =>
          qb
            .where(`${!req.minAmount}`)
            .orWhere('ware.amount >= :minAmount', { minAmount: req.minAmount }),
        ),
      )
      .andWhere(
        new Brackets((qb) =>
          qb
            .where(`${!req.maxAmount}`)
            .orWhere('ware.amount <= :maxAmount', { maxAmount: req.maxAmount }),
        ),
      )
      .andWhere(
        new Brackets((qb) =>
          qb
            .where(`${!req.minIntake}`)
            .orWhere('ware.intake >= :minIntake', { minIntake: req.minIntake }),
        ),
      )
      .andWhere(
        new Brackets((qb) =>
          qb
            .where(`${!req.maxIntake}`)
            .orWhere('ware.intake <= :maxIntake', { maxIntake: req.maxIntake }),
        ),
      )
      .andWhere(
        new Brackets((qb) =>
      ...
      .andWhere(
        new Brackets((qb) =>
          qb
            .where(`${!req.minDate}`)
            .orWhere('ware.createdAt >= :minDate', { minDate: req.minDate }),
        ),
      )
      .andWhere(
        new Brackets((qb) =>
