...
import { useExecuteShopDeliveryMutation } from './shops-deliveries.api';
...
export default function ExecuteShopDeliveryModal({
  data: shopDelivery,
}: Props) {
  ...
  const [executeShopDelivery, { isLoading }] = useExecuteShopDeliveryMutation();

  const handleSubmit = async (dto: ShopDeliveryIdDto) => {
    await executeShopDelivery(dto);
  };
...
export const executeShopDeliveryAction = {
  open: (shopDelivery: ShopDelivery) =>
    openModal({
      title: t('actions.execute') + ' ' + t('modals.deliveries'),
      children: <ExecuteShopDeliveryModal data={shopDelivery} />,
    }),
