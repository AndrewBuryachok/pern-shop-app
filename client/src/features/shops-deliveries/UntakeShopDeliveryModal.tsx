...
import { useUntakeShopDeliveryMutation } from './shops-deliveries.api';
...
export default function UntakeShopDeliveryModal({ data: shopDelivery }: Props) {
  ...
  const [untakeShopDelivery, { isLoading }] = useUntakeShopDeliveryMutation();

  const handleSubmit = async (dto: ShopDeliveryIdDto) => {
    await untakeShopDelivery(dto);
  };
...
export const untakeShopDeliveryAction = {
  open: (shopDelivery: ShopDelivery) =>
    openModal({
      title: t('actions.untake') + ' ' + t('modals.deliveries'),
      children: <UntakeShopDeliveryModal data={shopDelivery} />,
    }),
  disable: (shopDelivery: ShopDelivery) => shopDelivery.status !== Status.TAKEN,
  color: Color.RED,
};
